  ...
  where TEntity : class, IEntity, new()
{
  ...
  public async Task<IQueryable<TEntity>> GetAsync<TKey>(
    ...
    if (predicateExpression is not null) queryable = queryable.Where(predicateExpression);
    ...
    var skipped = skip.HasValue
                    ? queryable.Skip(skip.Value)
                    : queryable;
    var taken = take.HasValue
                  ? skipped.Take(take.Value)
                  : skipped;
