...
public class LocalizationService : ILocalizationService
{
  ...
  public string GetLocalization(LanguageType languageType, string key) {
    ...
    var localizationString =
      _unitOfWork.LocalizationStringRepository.GetFirstOrDefault(x => x.Language == languageType && x.Key == key);
  ...
  public string GetLocalization(LanguageType languageType, string key, Dictionary<string, object> parameters) {
    ...
    foreach (var parameter in parameters) formatter.AddArgument(parameter.Key, parameter.Value.ToString());
    return formatter.ToString();
  }

  public LanguagePackResultDTO GetLanguagePacks() {
    var languagePacks = _unitOfWork.LocalizationStringRepository
      .Get()
  ...
  public CustomResult AddOrUpdateLocalization(LocalizationString localizationString) {
    var loc = _unitOfWork.LocalizationStringRepository.GetFirstOrDefault(x =>
      x.Language == localizationString.Language && x.Key == localizationString.Key);
    if (loc == null) {
  ...
  public CustomResult DeleteLocalization(LanguageType languageType, string key) {
    var loc = _unitOfWork.LocalizationStringRepository.GetFirstOrDefault(
      x => x.Language == languageType && x.Key == key);
